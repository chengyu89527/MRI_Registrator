...
void ConvertAndScaleNewNiftiBuffer(char* buffer, T*& tbuffer, const NiftiHeader& niihdr, const size_t & imagesize);
  // read
template <class T>
int read_volume(volume<T>& target, const string& filename, const bool& legacyRead=true);
template <class T>
...
int read_volume_hdr_only(volume<T>& target, const string& filename) {
  ...
  NiftiHeader niihdr;
  try {
    niihdr = loadHeader(return_validimagefilename(filename));
...
template <class T>
int read_volume(volume<T>& target, const string& filename, const bool& legacyRead)
{
  short dtype;
  read_volumeROI(target,filename,dtype,0,0,0,0,-1,-1,-1,-1,true);
  if ( legacyRead && target.tsize() > 1 ) {
    cerr << "Warning: An input intended to be a single 3D volume has " <<
    "multiple timepoints. Input will be truncated to first volume, but " <<
    "this functionality is deprecated and will be removed in a future release." << endl;
...
struct typedSave {
  int operator()(const volume<T> source, const string& filename,const int filetype) {
    volume<dType> output;
...
int save_volume_dtype(const volume<T>& source, const string& filename,short datatype,const int filetype=-1)
{
  switch(closestTemplatedType(datatype)) {
    case DT_UNSIGNED_CHAR:  return typedSave<T,char>()(source,filename,filetype);
    case DT_SIGNED_SHORT:   return typedSave<T,short>()(source,filename,filetype);
    case DT_SIGNED_INT:     return typedSave<T,int>()(source,filename,filetype);
    case DT_FLOAT:          return typedSave<T,float>()(source,filename,filetype);
    case DT_DOUBLE:         return typedSave<T,double>()(source,filename,filetype);
    default:
