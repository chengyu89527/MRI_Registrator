...
// Needed to fix the sforms/qforms
// of the generated voxelwise EVs
// (one value per 2D slice of the
// functional data, which should
...
int do_work(int argc, char* argv[])
{
  ...
  volume4D<float> in_vol;
  read_volume_hdr_only(in_vol, inname.value());
  ...
  // save output - each EV has shape
  // (1, 1, nz, nv) (or nx/ny, depending
  // on slice orientation)
  ...
  // adjust the EV image affine so that
  // the scalar value within each
  // slice takes up the entire slice.
  // We update the pixdims, and the
  // sform and qform (unless they are
  // unknown)
  int sform_code = in_vol.sform_code();
  int qform_code = in_vol.qform_code();
  if (sform_code != NIFTI_XFORM_UNKNOWN ||
      qform_code != NIFTI_XFORM_UNKNOWN) {
    Matrix scaleoffset(IdentityMatrix(4));
    scaleoffset(1, 1) = in_vol.xsize();
    scaleoffset(2, 2) = in_vol.ysize();
    scaleoffset(3, 3) = in_vol.zsize();
    if      (slicedir.value() == "x") {
      scaleoffset(1, 1) = 1;
      scaleoffset(2, 4) = (in_vol.maxy() - in_vol.miny()) / 2.0;
      scaleoffset(3, 4) = (in_vol.maxz() - in_vol.minz()) / 2.0;
    }
    else if (slicedir.value() == "y") {
      scaleoffset(2, 2) = 1;
      scaleoffset(1, 4) = (in_vol.maxx() - in_vol.minx()) / 2.0;
      scaleoffset(3, 4) = (in_vol.maxz() - in_vol.minz()) / 2.0;
    }
    else if (slicedir.value() == "z") {
      scaleoffset(3, 3) = 1;
      scaleoffset(1, 4) = (in_vol.maxx() - in_vol.minx()) / 2.0;
      scaleoffset(2, 4) = (in_vol.maxy() - in_vol.miny()) / 2.0;
    }

    ev_vol.setdims(in_vol.xdim() * scaleoffset(1, 1),
                   in_vol.ydim() * scaleoffset(2, 2),
                   in_vol.zdim() * scaleoffset(3, 3),
                   in_vol.tdim());

    if (sform_code != NIFTI_XFORM_UNKNOWN) {
      ev_vol.set_sform(sform_code, in_vol.sform_mat() * scaleoffset);
    }
    if (qform_code != NIFTI_XFORM_UNKNOWN) {
      ev_vol.set_qform(qform_code, in_vol.qform_mat() * scaleoffset);
    }
  }
