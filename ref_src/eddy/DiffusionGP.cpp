					     ...
					     bool         exclude) EddyTry
{
  ...
  NEWMAT::RowVector pv = _Kmat->PredVec(indx,exclude); 
...
{
  if (n == _sptrs.size()) return; 
  else if (n > _sptrs.size()) {   
    _sptrs.resize(n,std::shared_ptr<NEWIMAGE::volume<float> >()); 
    _dpars.resize(n); 
    _Kmat->Reset();
    _pop = false;
  }
...
void DiffusionGP::predict_image_cpu(
				    unsigned int             indx,
				    bool                     exclude,
...
void DiffusionGP::predict_images_cpu(
