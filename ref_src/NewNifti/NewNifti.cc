...
namespace NiftiIO {
  ...
  void fileIO::readExtensions(const NiftiHeader& header, vector<NiftiExtension>& extensions)
  {
    ...
    while ( true ) {
      ...
      } catch ( NiftiException ) { //On a read error, return if reading pair or rethrow
        if ( header.singleFile() )
          throw; //Rethrow original exception
        return;
      }
      if ( header.wasWrongEndian )
        byteSwap(sizeof(iBuffer[0]), iBuffer, 8);
                ...
                for ( int64_t x = 0; x < header.dim[1]; x++ ) {
                  ...
                  if ( wasInROI != currentlyInROI ) { //We have switched so flush according to wasInROI
                    if ( wasInROI ) { //read the accumulated bytes
                      ...
                      reader.readRawBytes(movingBuffer, voxelsToRead*(header.bitsPerVoxel/8) );
                      movingBuffer+=voxelsToRead*(header.bitsPerVoxel/8);
                      voxelsToRead=0;
                    } else {
                      //cerr << "seeking " << voxelsToSeek << " voxels" << x << " " << y << " " << z <<endl;
                      reader.seek(voxelsToSeek*(header.bitsPerVoxel/8), SEEK_CUR );
                      voxelsToSeek=0;
                    }
