...
public:
  DataSelector(const std::vector<std::shared_ptr<NEWIMAGE::volume<float> > >& idata,
	       ...
	       unsigned int                                                   nvox,
	       bool                                                           rndinit=false) EddyTry { common_constructor(idata,mask,nvox,0.0,rndinit); } EddyCatch
  DataSelector(const std::vector<std::shared_ptr<NEWIMAGE::volume<float> > >& idata,
	       const NEWIMAGE::volume<float>&                                 mask,
	       unsigned int                                                   nvox,
	       float                                                          fwhm,
	       bool                                                           rndinit=false) EddyTry { common_constructor(idata,mask,nvox,fwhm,rndinit); } EddyCatch
  ~DataSelector() {}
...
public:
  ...
  virtual ~MMLHyParCF() EddyTry {} EddyCatch
...
public:
  ...
  virtual ~CVHyParCF() EddyTry {} EddyCatch
...
public:
  ...
  virtual ~GPPHyParCF() EddyTry {} EddyCatch
...
public:
  ...
  virtual bool RndInit() const { return(false); }
...
public:
  ...
  virtual ~FixedValueHyParEstimator() EddyTry {} EddyCatch
  ...
  virtual void Estimate(std::shared_ptr<const KMatrix>  K, bool verbose) EddyTry {
    if (verbose) cout << "Hyperparameters set to user specified values: " << get_hpar() << endl;
  } EddyCatch
};
...
public:
  CheapAndCheerfulHyParEstimator(unsigned int                      nvox=1000,
				 bool                              ir=false) EddyTry : _nvox(nvox), _ir(ir) {} EddyCatch
  virtual ~CheapAndCheerfulHyParEstimator() EddyTry {} EddyCatch
  ...
  virtual bool RndInit() const { return(_ir); }
  virtual void SetData(const std::vector<NEWMAT::ColumnVector>&    data) EddyTry { _data = data; } EddyCatch
...
public:
  ...
  virtual ~FullMontyHyParEstimator() EddyTry {} EddyCatch
