...
void PEASUtils::PostEddyAlignShells(
				    ...
				    ECScanManager&                  sm) EddyTry
{
  ...
  PEASUtils::align_shells_using_MI(clo,false,sm,grpb,mi_mov_par,mi_cmov_par,mi_mp_for_updates); 
...
void PEASUtils::PostEddyAlignShellsAlongPE(
					   const EddyCommandLineOptions&   clo,
					   bool                            upe, 
					   ...
					   ECScanManager&                  sm) EddyTry
{
  ...
  PEASUtils::align_shells_using_MI(clo,true,sm,grpb,mi_mov_par,mi_cmov_par,mi_mp_for_updates); 
...
void PEASUtils::WritePostEddyBetweenShellMIValues(
						  const EddyCommandLineOptions&     clo,
...
NEWIMAGE::volume<float> PEASUtils::get_mean_scan(
						 const EddyCommandLineOptions&     clo,      
						 const ECScanManager&              sm,
						 ...
						 NEWIMAGE::volume<float>&          mask) EddyTry
{
  ...
  for (unsigned int i=0; i<indx.size(); i++) {
    ...
    tmpmask = 1.0;
    mean += EddyGpuUtils::GetUnwarpedScan(sm.Scan(indx[i],ANY),sm.GetSuscHzOffResField(indx[i],ANY),false,sm.GetPolation(),&tmpmask);
    mask *= tmpmask;
  }
#else
...
NEWMAT::ColumnVector PEASUtils::register_volumes(
...
NEWMAT::ColumnVector PEASUtils::register_volumes_along_PE(
							  const NEWIMAGE::volume<float>& ref,
							  const NEWIMAGE::volume<float>& ima,
							  const NEWIMAGE::volume<float>& mask,
							  unsigned int                   pe_dir,
...
void PEASUtils::update_mov_par_estimates(
...
void PEASUtils::align_shells_using_MI(
				      const EddyCommandLineOptions&                      clo,
				      bool                                               pe_only,
...
void PEASUtils::align_shells_using_interspersed_B0_scans(
							 const EddyCommandLineOptions&                      clo,
							 ...
							 std::vector<NEWMAT::ColumnVector>&                 mp_for_updates) EddyTry
{
  ...
  EddyUtils::GetGroups(dpv,grpi,grpb);  
  std::vector<unsigned int> b0_indx = sm.GetB0Indicies();
  
  mov_par.resize(grpb.size()-1);
  for (unsigned int i=0; i<grpb.size()-1; i++) mov_par[i].ReSize(6);

  NEWMAT::ColumnVector delta(grpb.size());                 
  NEWMAT::ColumnVector n(grpb.size());                     
  for (unsigned int i=0; i<6; i++) { 
    delta=0.0; n=0.0;
    for (unsigned int j=0; j<b0_indx.size(); j++) { 
      if (b0_indx[j]==0) {
